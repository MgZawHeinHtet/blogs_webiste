        ...
        <div class="card-body">
            ...
            <p class="fs-10 text-secondary mb-0">
                <a class="text-secondary fw-bold text-decoration-none"
                    href="/blogs?author={{ $blog->user->username }}{{ request('category') ? '&category=' . request('category') : '' }}{{ request('search') ? '&search=' . request('search') : '' }}">{{ $blog->user->name }}</a>
                <span> -{{ $blog->created_at->diffForHumans() }}</span>
            </p>
            <div class="tags my-2">
                <span class="badge bg-primary"><a class="text-white text-decoration-none"
